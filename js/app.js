...
function renderizarCore() {
    ...
    // 2. TENDENCIAS DINÁMICAS (vs Periodo Anterior)
    calcularTendencias(eficacia, stats.conformes, stats.riesgo, stats.exceso, total);

    // 3. ANIMACIONES DE ALERTA (Respiración en Riesgos/Excesos si hay registros)
    const iconRiesgo = document.getElementById('icon-riesgo');
    const iconExceso = document.getElementById('icon-exceso');
...
function calcularTendencias(efActual, confActual, riesActual, excActual, totActual) {
    ...
    const lblRi = document.getElementById('trend-riesgo'); const lblEx = document.getElementById('trend-exceso'); const lblTo = document.getElementById('trend-total');
    ...
    prevDatos.forEach(r => {
        ...
        if (p) {
            const val = parseConcen(r.concen);
            if (val < p.min) pStats.r++; else if (val > p.max) pStats.e++; else pStats.c++;
        }
    });
    
    let pTot = prevDatos.length; let pEf = pTot > 0 ? (pStats.c / pTot) * 100 : 0;

    const render = (el, actual, prev, isPct, invertColors = false) => {
        if(!el) return;
        let diff = actual - prev;
        if(pTot === 0) { el.innerText = "Sin datos previos"; el.className = "text-[10px] font-bold mt-1 text-slate-400"; return; }
        
        let prefix = diff > 0 ? '▲ +' : (diff < 0 ? '▼ ' : '■ ');
        let colorClass = 'text-slate-400';
        if (diff !== 0) {
            if (invertColors) { colorClass = diff > 0 ? 'text-red-500' : 'text-emerald-500'; } // Para riesgos/excesos subir es malo
            else { colorClass = diff > 0 ? 'text-emerald-500' : 'text-red-500'; } // Para eficacia subir es bueno
        }
        
        let valStr = isPct ? diff.toFixed(1) + '%' : diff;
        el.innerText = `${prefix}${valStr} vs Periodo Ant.`;
        el.className = `text-[10px] font-bold mt-1 ${colorClass}`;
    };
    ...
    render(lblTo, totActual, pTot, false, false); // Neutro, o podemos ponerlo invertColors = false
}

// SPARK LINES PREMIUM (Áreas, Histogramas y Promedios)
function drawSparklines(datos) {
    if(datos.length === 0) {
        ['sparkEficacia', 'sparkConformes', 'sparkRiesgo', 'sparkExceso', 'sparkTotal'].forEach(id => {
            if(sparkInst[id]) sparkInst[id].destroy();
        });
        return;
    }

    // Agrupación por Fecha (o por Hora si solo hay 1 fecha seleccionada)
    ...
    datos.forEach(r => {
        ...
        if(!grouped[key]) grouped[key] = { t: 0, c: 0, r: 0, e: 0 };
        grouped[key].t++;
        let p = PARAMETROS_TECNICOS.find(x => x.solucion === r.solucion);
        if(p) {
            let v = parseConcen(r.concen);
            if(v < p.min) grouped[key].r++; else if(v > p.max) grouped[key].e++; else grouped[key].c++;
        }
    });
    ...
    let dTot = keys.map(k => grouped[k].t);
    ...
    const baseOpts = { 
        ...
        plugins: { 
            ...
            tooltip: { enabled: true, mode: 'index', intersect: false, displayColors: false, titleFont: {size: 9}, bodyFont: {size: 10, weight: 'bold'}, padding: 6, backgroundColor: 'rgba(30, 41, 59, 0.9)' } 
        }, 
    ...
    function renderSpark(id, type, data, color, isFill = false) {
        ...
        if (type === 'line' && isFill) {
            ...
            grad.addColorStop(0, color.replace('1)', '0.3)')); // Semi-transparente arriba
            grad.addColorStop(1, color.replace('1)', '0)'));   // Transparente abajo
    ...
    // Eficacia y Totales -> Áreas Suavizadas
    renderSpark('sparkEficacia', 'line', dEfi, 'rgba(59, 130, 246, 1)', true); // Azul
    renderSpark('sparkTotal', 'line', dTot, 'rgba(148, 163, 184, 1)', true); // Gris
    
    // Conformes, Riesgo, Exceso -> Histogramas (Barras)
    renderSpark('sparkConformes', 'bar', dConf, 'rgba(16, 185, 129, 1)'); // Verde
    renderSpark('sparkRiesgo', 'bar', dRies, 'rgba(239, 68, 68, 1)'); // Rojo
    renderSpark('sparkExceso', 'bar', dExc, 'rgba(245, 158, 11, 1)'); // Amarillo
}

// RESTO DEL CÓDIGO INTACTO (Mapa Calor, Cuadrante, Drilldown, etc...)
...
function drawHeatmapOperativo(datos, containerId = 'heatmap-container', isExpanded = false) {
    ...
    dias.forEach(d => { html += `<th class="${paddingSize} border-b border-slate-200">${d.label}</th>`; }); 
...
function drawTendenciaHistorica(datos, canvasId = 'historicoChart', isExpanded = false) {
    ...
    datos.forEach(r => { 
        ...
        if(p) { 
            ...
            hist[r.fecha].total++; let v = parseConcen(r.concen); 
            if(v < p.min) hist[r.fecha].riesgos++; else if(v > p.max) hist[r.fecha].excesos++; else hist[r.fecha].conformes++; 
        } 
    });
...
function drawMagicQuadrant(datos, canvasId = 'quadrantChart', isExpanded = false) {
    ...
    let evalSoluciones = {}; 
    datos.forEach(r => { let p = PARAMETROS_TECNICOS.find(x=>x.solucion===r.solucion); if(p) { if(!evalSoluciones[r.solucion]) evalSoluciones[r.solucion] = { total: 0, ok: 0 }; evalSoluciones[r.solucion].total++; let v = parseConcen(r.concen); if(v >= p.min && v <= p.max) evalSoluciones[r.solucion].ok++; } });
    let scatterData = []; let tooltipsData = []; 
    Object.keys(evalSoluciones).forEach(sol => { let vol = evalSoluciones[sol].total; let efi = (evalSoluciones[sol].ok / vol) * 100; scatterData.push({ x: efi, y: vol }); tooltipsData.push(sol); });
...
function expandirGrafico(tipo, titulo) {
    document.getElementById('expandido-titulo').innerHTML = `<i class="fa-solid fa-expand text-blue-500 mr-2"></i> Vista Detallada: ${titulo}`; document.getElementById('modal-expandido').classList.remove('hidden');
    const canvas = document.getElementById('expandidoChart'); const div = document.getElementById('expandidoDiv'); canvas.classList.add('hidden'); div.classList.add('hidden'); const datos = obtenerDatosFiltrados();
    if(tipo === 'heatmap') { div.classList.remove('hidden'); drawHeatmapOperativo(datos, 'expandidoDiv', true); } 
    else { canvas.classList.remove('hidden'); if(tipo === 'cuadrante') drawMagicQuadrant(datos, 'expandidoChart', true); if(tipo === 'historico') drawTendenciaHistorica(datos, 'expandidoChart', true); if(tipo === 'barras') drawEficaciaSoluciones(datos, 'expandidoChart', true); if(tipo === 'radar') drawRadarFugas(desviosUltimoFiltro, 'expandidoChart', true); }
}
...
function clicRadiografiaTurno(franjaKey, diaKey, diaLabel) {
    let lista = heatmapCache[franjaKey] && heatmapCache[franjaKey][diaKey] ? heatmapCache[franjaKey][diaKey] : []; if(lista.length === 0) return; 
    let dictQuimicos = {}; let dictOperadores = {}; let dictEquipos = {}; 
    lista.forEach(r => { dictQuimicos[r.solucion] = (dictQuimicos[r.solucion] || 0) + 1; dictOperadores[r.operario || 'Sin nombre'] = (dictOperadores[r.operario || 'Sin nombre'] || 0) + 1; dictEquipos[r.equipo || 'Sin equipo'] = (dictEquipos[r.equipo || 'Sin equipo'] || 0) + 1; }); 
    document.getElementById('turno-titulo').innerHTML = `<i class="fa-solid fa-clipboard-user mr-2"></i> Radiografía Operativa: ${franjaKey.toUpperCase()} (${diaLabel.toUpperCase()})`; document.getElementById('modal-turno').classList.remove('hidden');
...
async function dispararAnalisisIA() {
    ...
    try {
        ...
        if (!res.ok) throw new Error(await res.text()); const jsonRes = await res.json(); let plan = JSON.parse((jsonRes.candidates?.[0]?.content?.parts?.[0]?.text || '').replace(/```json|```/gi, '').trim());
