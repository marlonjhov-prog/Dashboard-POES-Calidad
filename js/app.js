...
function n(t) { 
    return t ? String(t).trim().toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") : ''; 
}
...
async function cargarSupabase() {
    ...
    listaRegistros = acumulador.map(r => ({ 
        ...r, 
        solucion: estandarizarSolucion(r.solucion), 
        equipo: String(r.equipo || 'N/A').trim(), 
        proceso: String(r.proceso || 'CIP').trim().toUpperCase() 
    }));
    ...
    fpInstancia = flatpickr("#filtro-fechas", {
        ...
        onChange: function(selectedDates, dateStr, instance) {
            if (selectedDates.length === 2) {
                fechaInicioGlobal = instance.formatDate(selectedDates[0], "Y-m-d"); 
                fechaFinGlobal = instance.formatDate(selectedDates[1], "Y-m-d");
...
function drawTendenciaHistorica(datos, canvasId = 'historicoChart', isExpanded = false) {
    let targetInst = isExpanded ? expandedChartInst : historicoInst; 
    ...
    let newInst = new Chart(document.getElementById(canvasId).getContext('2d'), {
        ...
        data: { 
            ...
            datasets: [ 
                { type: 'line', label: 'Eficacia (%)', data: arrEficacias, borderColor: '#273c75', borderWidth: isExpanded ? 4 : 2, fill: false, tension: 0.3, pointRadius: isExpanded ? 6 : 4, pointBackgroundColor: '#273c75', yAxisID: 'porcentaje', order: 0 }, 
                { label: 'Conformes', data: arrConformes, backgroundColor: '#10b981', stack: 'Stack 0', yAxisID: 'volumen', order: 1 }, 
                { label: 'Exceso', data: arrExcesos, backgroundColor: '#f59e0b', stack: 'Stack 0', yAxisID: 'volumen', order: 1 }, 
                { label: 'Riesgo', data: arrRiesgos, backgroundColor: '#ef4444', stack: 'Stack 0', yAxisID: 'volumen', order: 1 } 
            ] 
        },
        options: { 
            ...
            scales: { 
                ...
                porcentaje: { type: 'linear', position: 'right', min: 0, max: 100, grid: { drawOnChartArea: false }, ticks: { callback: v => v + '%', font: {size: isExpanded?12:10} } } 
            } 
        }
    });
...
function drawMagicQuadrant(datos, canvasId = 'quadrantChart', isExpanded = false) {
    let targetInst = isExpanded ? expandedChartInst : quadrantInst; 
    if(targetInst) targetInst.destroy(); 
...
function drawEficaciaSoluciones(datos, canvasId = 'barSolucionesChart', isExpanded = false) {
    let targetInst = isExpanded ? expandedChartInst : barSolucionesInst; 
    if(targetInst) targetInst.destroy();
...
function drawRadarFugas(desvios, canvasId = 'fugaQuimicaChart', isExpanded = false) {
    let targetInst = isExpanded ? expandedChartInst : fugaChartInst; 
    if(targetInst) targetInst.destroy();
...
function clicRadiografiaTurno(franjaKey, diaKey, diaLabel) {
    ...
    turnoQuimInst = new Chart(document.getElementById('turnoQuimicosChart').getContext('2d'), { 
        type: 'doughnut', 
        data: { labels: Object.keys(dictQuimicos), datasets: [{ data: Object.values(dictQuimicos), backgroundColor: ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4'], borderWidth: 2, borderColor: '#ffffff' }] }, 
