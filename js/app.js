...
// ==========================================
// PLUGIN CUADRANTE MÁGICO (TEXTOS CORREGIDOS)
// ==========================================
const quadrantPlugin = {
    ...
    beforeDraw(chart) {
        ...
        ctx.save();
        ctx.lineWidth = 1; ctx.strokeStyle = '#cbd5e1'; ctx.setLineDash([4, 4]);
        ...
        if(midX > left) {
            ...
            ctx.fillText('NICHO (CONTROLADO)', right - 10, midY + 15);
        }
        ctx.restore();
    }
};
...
function initFiltrosInteligentes() {
    ['filtro-equipo', 'filtro-solucion', 'filtro-anio', 'filtro-mes'].forEach(id => { tsInstances[id] = new TomSelect(`#${id}`, { create: false, sortField: { field: "text", direction: "asc" } }); tsInstances[id].on('change', renderizarCore); });
}
    ...
    datos.forEach(r => {
        ...
        if(p) {
            ...
            if(fMatch && matriz[fMatch.key] && matriz[fMatch.key][dKey] !== undefined) {
                matriz[fMatch.key][dKey].push({ ...r, estado });
            }
        }
    });
    ...
    franjas.forEach(f => {
        ...
        dias.forEach(d => {
            ...
            if(count > 0) {
                ...
                if(hasRiesgo) {
                    bgClass = 'bg-red-100 text-red-800 border border-red-300 font-bold cursor-pointer hover:bg-red-200';
                } else if(hasExceso) {
                    bgClass = 'bg-amber-100 text-amber-800 border border-amber-300 font-bold cursor-pointer hover:bg-amber-200';
                } else {
...
function drawMagicQuadrant(datos) {
    ...
    datos.forEach(r => {
        ...
        if(p) { if(!evalSoluciones[r.solucion]) evalSoluciones[r.solucion] = { total: 0, ok: 0 }; evalSoluciones[r.solucion].total++; let v = parseConcen(r.concen); if(v >= p.min && v <= p.max) evalSoluciones[r.solucion].ok++; }
    });
    ...
    quadrantInst = new Chart(document.getElementById('quadrantChart').getContext('2d'), {
        ...
        options: {
            ...
            scales: { x: { title: { display: true, text: 'Eficacia Sanitaria (%)', font: {size: 10, weight: 'bold'}, color: '#64748b' }, min: 0, max: 100, grid: { display: false } }, y: { title: { display: true, text: 'Volumen Operativo (Muestras)', font: {size: 10, weight: 'bold'}, color: '#64748b' }, min: 0, grid: { display: false } } }
        }
    });
}
...
function abrirModalDrilldown(solucion) {
    ...
    drilldownInst = new Chart(document.getElementById('drilldownChart').getContext('2d'), {
        type: 'line',
        data: {
            labels: subset.map(r => `${r.fecha.substring(5)} ${r.hora?r.hora.substring(0,5):''}`),
            datasets: [
                { label: 'Muestras', data: dataPoints, showLine: false, pointBackgroundColor: colors, pointBorderColor: '#ffffff', pointBorderWidth: 1.5, pointRadius: 6, pointHoverRadius: 9 },
                { label: 'Max', data: Array(subset.length).fill(regla.max), borderColor: '#f59e0b', borderDash: [5,5], pointRadius: 0, fill: false, borderWidth: 2 },
...
async function dispararAnalisisIA() {
    ...
    let excesos = desviosUltimoFiltro.filter(d => d.tipo.includes('Exceso')); let totalExcesos = excesos.length;
    ...
    let desgloseTexto = `TOTAL EVENTOS POR EXCESO: ${totalExcesos}\n`;
    Object.keys(statsFugas).forEach(sol => {
        let pct = ((statsFugas[sol].conteo / totalExcesos) * 100).toFixed(1);
        desgloseTexto += `- Químico ${sol}: ${pct}% de los eventos (${statsFugas[sol].conteo} veces). Volumen Desperdiciado (Índice de fuga): ${statsFugas[sol].volumenPerdido.toFixed(2)}.\n`;
    });
    let opsArray = Object.keys(opsStats).map(op => `${op} (${opsStats[op]} eventos)`).join(', ');

    const prompt = `Eres Analista de Datos en Lácteos San Antonio. Analiza ESTOS DATOS DUROS:\n\n${desgloseTexto}\nOperadores implicados: ${opsArray}\n\nREGLAS ESTRICTAS:\n1. Usa los porcentajes exactos provistos.\n2. Explica qué químico representa el mayor desperdicio.\n3. PROHIBIDO dar recomendaciones mecánicas.\n4. Devuelve un JSON estricto: [{"desvio": "Hallazgo principal", "analisis_datos": "Análisis estadístico", "responsable": "Nombre del operario"}]. Sin markdown.`;
